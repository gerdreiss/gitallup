...
listReposAndRest root = makeAbsolute root >>= listDirectories >>= partitionM Git.isGitRepo

listDirectories :: FilePath -> IO [FilePath]
listDirectories path = ifM (doesDirectoryExist path) (fmap (path </>) <$> listDirectory path) (return [])
...
listNestedRepos recursive depth only excluded subdirs
  ...
  <$> sequence (map (listRepos True (depth - 1) only excluded) subdirs `using` parList rpar)
  | otherwise = return []
...
checkStatusCleanupDeleteBranchesOrUpdateRepos repos = ifM
  (view statusL)
...
cleanupRepo repo = Log.logRepo repo >> RepoUpdateResult repo Nothing <$> Git.cleanRepo repo
...
checkStatusRepos repos = sequence (map checkStatusRepo repos `using` parList rpar)

checkStatusRepo :: FilePath -> RIO App RepoUpdateResult
checkStatusRepo repo = Log.logRepo repo >> RepoUpdateResult repo Nothing <$> Git.branchStatus repo

--
-- Update
--
updateRepos :: [FilePath] -> RIO App [RepoUpdateResult]
updateRepos repos = concat <$> sequence (map updateRepo repos `using` parList rpar)
...
checkIsDirtyAndHardResetCurrentBranch repo = ifM (isCurrentBranchDirty repo)
...
hardResetCurrentBranch repo =
  Log.debug "Retrieving the current branch..." --
    ...
    >>= either (errorResult repo) (maybe (noCurrentBranchErrorResult repo) (hardResetBranch repo))

hardResetBranch :: FilePath -> B.ByteString -> RIO App RepoUpdateResult
hardResetBranch repo branch =
  RepoUpdateResult repo (Just branch) <$> (Log.debug ("Hard reset the branch " ++ C8.unpack branch) >> Git.resetHard repo branch)

checkCurrentBranchSwitchUpdate :: FilePath -> RIO App RepoUpdateResult
checkCurrentBranchSwitchUpdate repo =
  Log.debug "Retrieving the current branch..." --
    >>  Git.currentBranch repo
    >>= either (errorResult repo) (maybe (noCurrentBranchErrorResult repo) (checkBranchNotMainSwitchUpdate repo))

checkBranchNotMainSwitchUpdate :: FilePath -> B.ByteString -> RIO App RepoUpdateResult
checkBranchNotMainSwitchUpdate repo branch =
  Log.debug "Checking if current branch is the main branch..." --
    ...
    >>= either
          (errorResult repo)
          (\isMainBranch -> if isMainBranch
            then generalSuccessResult repo (Just branch) "It's the main branch." --
            else retrieveMainBranchSwitchUpdate repo
          )
...
retrieveMainBranchSwitchUpdate repo =
  Log.debug "Retrieving the main/master branch..." --
    >>  Git.mainBranch repo
    >>= either (errorResult repo) (maybe (noMainBranchErrorResult repo) (switchBranchUpdate repo))
...
typedSuccessResult repo branch text resultType = return $ RepoUpdateResult
  ...
  , updateErrorOrSuccess = Right GitOpResult { resultType = resultType, resultText = text }
  }
...
noCurrentBranchErrorResult repo = return $ RepoUpdateResult
  ...
  , updateErrorOrSuccess = Left GitOpError { errorCode = -1, errorMessage = message }
  }
